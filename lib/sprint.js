...
(function() {
  ...
  var selectElements = function(selector, context) {
    // 如果没有 context 就设置为 document
    context = context || document
    // class, id, tag name or universal selector
    // 类名，id名，标签名，或 普遍选择器，只能检查简单的 li.box 就查找不到
    // /^[\#.]?[\w-]+$/  正则检测是否符合选择器规则
    // [#.]   开始 #
    ...
    */
    /** 统一调用 querySelectorAll */
    // toArray 转换成数组，核心方法之一    G:532
    // 最后返回 querySelectorAll() 查找的元素 返回 G：635
  ...
  // toArray() 把 NodeList 转换成 普通数组 的方法
  var toArray = function(obj) {
    // obj 是 querySelector 查找到的 NodeList 
    var arr = []
    // i 就是 NodeList 的长度
    var i = obj.length
    // while 的好处 循环到 0 自动退出循环 
    while (i--) {
      // 浅拷贝
      arr[i] = obj[i]
    }
  ...
  // constructor
  // 初始化的核心方法， selector 就是选择器
  var Init = function(selector, context) {
    // 先检查传入的是否为字符串，如果是，创建元素 或者 选择元素
    if (typeof selector == "string") {
      // create DOM element   ,创建元素，识别字符串的 首字是否为 < 如 $("<div>")
      if (selector[0] == "<") {
        this.dom = [createDOM(selector)]
      }
      // select DOM elements，选择 DOM 元素，
      // this.dom 存放 dom 元素
      else { 
        // 这里的三元表达式
        this.dom = 
          // 如果有  context，检查是否为 Init 的实例
          context && context instanceof Init
          // 如果是，调用 find() 查找方法
          ? context.find(selector).get()
          // 如果没有传入 ，调用 selectElements 选择元素  G:498
  ...
  Init.prototype = {
    ...
    attr: function(name, value) {
      // 判断第二个参数是否为 函数，如果是函数，执行其他业务逻辑
      var isFunc = typeof value == "function"
      if (typeof value == "string" || typeof value == "number" || isFunc) {
        // return 
        return this.each(function(i) {
          // 判断当前遍历 this 是否为 元素，元素 nodeType === 1
          if (this.nodeType > 1) return
          // 遍历调用 setAttribute 方法
          this.setAttribute(
            // 如果是函数，调用函数，把函数返回值作为 value
  ...
  Sprint = function(selector, context) {
    // 调用 Init() 方法，初始化一个 $ 实例对象，最终作为返回值返回，调用后取到 607 行
